...
def run_pipeline() -> None:
    ...
    # ------------------------------------------------------------------
    ctx = new_run_context()
    pipeline_paths = build_paths(ctx)
    ensure_run_dirs(pipeline_paths)

    # ------------------------------------------------------------------
    # 2) Logger (observabilidade)
    # ------------------------------------------------------------------
    log_file = pipeline_paths.logs_dir / "pipeline.log"
    ...
    try:
        ...
        # 5) Persistência
        # ------------------------------------------------------------------
        write_parquet(df_silver, pipeline_paths.silver_file("orders"))
        write_parquet(df_rejects, pipeline_paths.rejects_file("orders"))

        df_metrics = pd.DataFrame([metrics])
        write_parquet(df_metrics, pipeline_paths.metrics_file())
